...
source ${DIR_BIN}/detect-os.sh
source ${DIR_BIN}/lib.sh
...
deploy_doc_to_s3() {
    local TMP_PATH_SPHINX_DOC_BUILD_HTML="$1"
    local TMP_S3_URI_DOC="$2"
...
get_pkg_version()
{
    ...
    PACKAGE_VERSION=$(${TMP_BIN_PIP} show ${TMP_PACKAGE_NAME} | grep Version)
    PACKAGE_VERSION=${PACKAGE_VERSION:9:100}
}


create_virtualenv()
{
    local TMP_BIN_VIRTUALENV=$1
    local TMP_BIN_PYTHON=$2
    local TMP_DIR_VENV=$3
}
